...
fn update_game_time(
    mut q_game_time_text: Query<Entity, With<GameTimeText>>,
    mut writer: TextUiWriter,
    game_time: Res<GameTime>,
) {
    for mut text in &mut q_game_time_text {
        *writer.text(text, 0) = format_game_time(game_time.0);
    }
}
...
fn player_stop(
      ...
      if *state == GameState::Death || *state == GameState::Victory {
...
fn player_about(
      ...
      if keyboard.pressed(KeyCode::ArrowLeft) {
          linear.x = -1. * 10000. * delta.delta_secs();
